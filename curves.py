...
class CurveSet(dict):
    ...
    def run_graph(self):
        ...
        cmd = "python3 %s %s \"%s\" %s" % (mod, self.steps, self.curves, self.name)
        os.system(cmd)
...
class NameCurve(object):
    ...
    def __init__(self, name, steps=5000, pause=None):
        ...
        self.name = ''.join(re.split("\s+", name.lower()))
        self.name_consonants = ''.join([ch for ch in self.name if ch not in self.Vowels])
        self.name_vowels = ''.join([ch for ch in self.name if ch in self.Vowels])
